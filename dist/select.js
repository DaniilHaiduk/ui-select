 ...
 * Version: 0.19.8 - 2020-05-20T11:19:06.502Z
 * License: MIT
 */
...
uis.directive('uiSelectSingle', ['$timeout','$compile', function($timeout, $compile) {
  return {
    ...
    link: function(scope, element, attrs, ctrls) {
      ...
      scope.$on('uis:activate', function () {
        // focusser.prop('disabled', true); //Will reactivate it on .close() remove focusser
      });
      /* Remove focusser
...
angular.module("ui.select").run(["$templateCache", function($templateCache) {$templateCache.put("bootstrap/choices.tpl.html","<ul class=\"ui-select-choices ui-select-choices-content\"><li class=\"ui-select-choices-group\" id=\"ui-select-choices-{{ $select.generatedId }}\"><div class=\"divider\" ng-show=\"$select.isGrouped && $index > 0\"></div><div ng-show=\"$select.isGrouped\" class=\"ui-select-choices-group-label dropdown-header\" ng-bind=\"$group.name\"></div><div ng-attr-id=\"ui-select-choices-row-{{ $select.generatedId }}-{{$index}}\" class=\"ui-select-choices-row\" ng-class=\"{active: $select.isActive(this), disabled: $select.isDisabled(this)}\" role=\"option\" aria-selected=\"{{ $select.isActive(this) }}\"><span class=\"ui-select-choices-row-inner\"></span></div></li></ul>");
...
$templateCache.put("bootstrap/match.tpl.html","<button tabindex=\"0\" class=\"ui-select-match\" ng-hide=\"$select.open && $select.searchEnabled\" ng-disabled=\"$select.disabled\" ng-class=\"{\'btn-default-focus\':$select.focus}\" ng-click=\"$select.activate(false, false, $event)\" aria-label=\"{{ ($select.selectorAriaLabel || $select.selected.title) + \' activate\' }}\" aria-haspopup=\"listbox\" aria-expanded=\"{{ $select.open }}\" aria-required=\"{{ $select.selectorAriaRequired }}\" id=\"selector-close-wrapper-{{ $select.generatedId }}\"><span tabindex=\"-1\" class=\"btn btn-default form-control ui-select-toggle\" aria-label=\"{{ $select.selected.title }} selected\" ng-disabled=\"$select.disabled\" ng-click=\"$select.activate()\" style=\"outline: 0;\"><span ng-show=\"$select.isEmpty()\" class=\"ui-select-placeholder text-muted\">{{$select.placeholder}}</span> <span ng-hide=\"$select.isEmpty()\" class=\"ui-select-match-text pull-left\" ng-class=\"{\'ui-select-allow-clear\': $select.allowClear && !$select.isEmpty()}\" ng-transclude=\"\"></span> <i class=\"caret pull-right\" ng-click=\"$select.toggle($event)\"></i> <a ng-show=\"$select.allowClear && !$select.isEmpty() && ($select.disabled !== true)\" aria-label=\"{{ $select.baseTitle }} clear\" style=\"margin-right: 10px\" ng-click=\"$select.clear($event)\" class=\"btn btn-xs btn-link pull-right\"><i class=\"glyphicon glyphicon-remove\" aria-hidden=\"true\"></i></a></span></button>");
