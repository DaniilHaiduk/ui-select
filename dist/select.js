 ...
 * Version: 0.19.8 - 2020-05-19T15:33:16.545Z
 * License: MIT
 */
...
$templateCache.put("bootstrap/match.tpl.html","<button tabindex=\"0\" class=\"ui-select-match\" ng-hide=\"$select.open && $select.searchEnabled\" ng-disabled=\"$select.disabled\" ng-class=\"{\'btn-default-focus\':$select.focus}\" ng-click=\"$select.activate(false, false, $event)\" aria-label=\"{{ ($select.selectorAriaLabel || $select.selected.title) + \' activate\' }}\" aria-haspopup=\"listbox\" aria-expanded=\"{{ $select.open }}\" aria-required=\"{{ $select.selectorAriaRequired }}\" id=\"selector-close-wrapper\"><span tabindex=\"-1\" class=\"btn btn-default form-control ui-select-toggle\" aria-label=\"{{ $select.selected.title }} selected\" ng-disabled=\"$select.disabled\" ng-click=\"$select.activate()\" style=\"outline: 0;\"><span ng-show=\"$select.isEmpty()\" class=\"ui-select-placeholder text-muted\">{{$select.placeholder}}</span> <span ng-hide=\"$select.isEmpty()\" class=\"ui-select-match-text pull-left\" ng-class=\"{\'ui-select-allow-clear\': $select.allowClear && !$select.isEmpty()}\" ng-transclude=\"\"></span> <i class=\"caret pull-right\" ng-click=\"$select.toggle($event)\"></i> <a ng-show=\"$select.allowClear && !$select.isEmpty() && ($select.disabled !== true)\" aria-label=\"{{ $select.baseTitle }} clear\" style=\"margin-right: 10px\" ng-click=\"$select.clear($event)\" class=\"btn btn-xs btn-link pull-right\"><i class=\"glyphicon glyphicon-remove\" aria-hidden=\"true\"></i></a></span></button>");
